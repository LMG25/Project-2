...
function Speed(pos) {
    this.basePos = this.pos = pos.plus(new Vector(0.2, 0.5));
    this.size = new Vector(1, 1);
    this.wobble = Math.PI * 2;
}
...
Golem.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
Flame.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  if (obstacle === "teleporter") {
	  this.pos = new Vector(75, 5);
  }
  if (obstacle === "slow") {
	  playerXSpeed = 7;
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "golem" && this.status == null) {
	  this.status = "lost";
	  this.finishDelay = 1;
  } else if (type == "flame" && this.status == null) {
	  this.status = "lost";
	  this.finishDelay = 1;
  } else if (type == "coin" || type == "speed") {
